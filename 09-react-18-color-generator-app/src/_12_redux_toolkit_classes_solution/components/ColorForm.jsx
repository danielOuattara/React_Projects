...
class ColorForm extends Component {
  render() {
    ...
    const handleUserColorChange = (value) => {
      if (userColor) {
        this.props.dispatch(colorGeneratorActions.setColorInputError(false));
      }
      this.props.dispatch(colorGeneratorActions.setUserColor(value));
    };

    const handleStepValue = (value) => {
      this.props.dispatch(colorGeneratorActions.setStep(value));
    };
    ...
    const handleSubmit = (event) => {
      ...
      try {
        ...
        this.props.dispatch(colorGeneratorActions.setList(newList));
      } catch (error) {
        this.props.dispatch(colorGeneratorActions.setColorInputError(true));
        this.props.dispatch(colorGeneratorActions.setUserColor(""));
      }
    };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <label htmlFor="step" className="label-step">
            ...
            <input
              ...
              min="1"
              value={step}
              onChange={(event) => handleStepValue(Number(event.target.value))}
            />
          </label>
