/* 
class + showAlert helper function + Alert as PureComponent 
*/

import { Component } from "react";
import { List, Alert } from "./components";

function retrieveLocalStorage() {
  let itemsList = localStorage.getItem("itemsListClassV3");
  if (itemsList) {
    return JSON.parse(localStorage.getItem("itemsListClassV3"));
  } else {
    return [];
  }
}

export default class AppClassV3 extends Component {
  state = {
    itemName: "",
  ...
  render() {
    return (
      <section className="section-center ">
        <p>class solution version 3 : showAlert() + Alert as PureComponent</p>

        <form className="grocery-form" onSubmit={this.handleSubmitItem}>
          <Alert alert={this.state.alert} showAlert={this.showAlert} />
          <h3>grocery bud</h3>
          <div className="form-control">
            <input
              ...
              onChange={(event) =>
                this.handleChangeItemName(event.target.value)
              }
            />
